...
@section('page_title', 'AttributeGroup')
...
<div class="px-5 bg-background w-full">
    ...
    <div class="flex justify-between">
        ...
        <div class="text-2xl font-bold">Attribute Group</div>

                <a href='{{ route('attributegroup.create') }}'
    ...
    <div class='product-table bg-white p-3 rounded-lg mt-10 font-main font-light shadow'>
        ...
        <div class="relative overflow-x-auto">
            <table class="w-full divide-y divide-gray-200">
                ...
                @foreach ($attribute_group as $key => $attributegroup)

                <tbody class="bg-white divide-y divide-gray-200 text-center">
                    <tr>
                        <td class="">
                            <div>{{ $attributegroup->attributename}}</div>
                        </td>
                        <td class="">
                            {{-- <div>{{ $attributegroup->attributename }}</div> --}}
                            @foreach ($sub_group as $sub)
                                @if ($sub->attribute_group === $attributegroup->id)
                                    <div>{{ $sub->name }}</div>
                                @endif
                            @endforeach
                        </td>
                        {{-- <td class="">
                            <div>{{ $attributegroup->attribute_order}}</div>
                        </td> --}}


                        <td> @if ($attributegroup->created_at)
                            <div>{{ $attributegroup->created_at->format('F j, Y') }}</div>
                            @else
                            <div>-</div>
                            @endif
                        </td>
                        ...
                        <td>
                            <div class="flex p-2 justify-center items-center">
                                ...
                                <a href='{{ route('attributegroup.show', $attributegroup->id)}}'
                                    >
                                ...
                                {{-- <a href="
                                    {{ route('attributegroup.edit', $attributegroup->id)}}
                                    ">
                                    <div class="bg-slate-500 py-1 px-2 mx-2 text-white flex rounded-md">
                                ...
                                <form action="
                                {{ route('attributegroup.destroy', $attributegroup->id) }}
                                " class="delete-form-{{ $attributegroup->id}}" method="post">
                                    @csrf
                                    @method('delete')
                                    <button type="button" class="bg-red-500 py-1 px-2 mx-2 mt-4 flex text-white rounded-md"
                                        onclick="deleteItem('{{ $attributegroup->id}}')">
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                            class="icon icon-tabler icon-tabler-trash" width="24" height="24"
                                            viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none"
                                            stroke-linecap="round" stroke-linejoin="round">
