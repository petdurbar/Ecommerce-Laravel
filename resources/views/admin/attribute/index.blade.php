...
<div class="px-5 bg-background w-full">
    ...
    <div class="flex justify-between">
        ...
        <div class="text-2xl font-bold">Attribute </div>
            ...
            <table class="w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200 text-center">
                    <tr>
                        ...
                        <td class="">
                            <div>{{$attributes->getgroup->attributename}}</div>
                        </td>
                        <td class="">
                            <div>{{$attributes->order}}</div>
                        </td>


                        <td> @if ($attributes->created_at)
                            <div>{{ $attributes->created_at->format('F j, Y') }}</div>
                            @else
                            <div>-</div>
                            @endif
                        </td>
                        ...
                        <td>
                            <div class="flex p-2 justify-center items-center">
                                ...
                                <a href='{{ route('attribute.show', $attributes->id)}}'
                                    >
                                ...
                                {{-- <a href="
                                    {{ route('attribute.edit', $attributes->id)}}
                                    ">
                                    <div class="bg-slate-500 py-1 px-2 mx-2 text-white flex rounded-md">
                                ...
                                <form action="
                                {{ route('attribute.destroy', $attributes->id) }}
                                " class="delete-form-{{ $attributes->id}}" method="post">
                                    @csrf
                                    @method('delete')
                                    <button type="button" class="bg-red-500 py-1 px-2 mx-2 mt-4 flex text-white rounded-md"
                                        onclick="deleteItem('{{ $attributes->id}}')">
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                            class="icon icon-tabler icon-tabler-trash" width="24" height="24"
                                            viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none"
                                            stroke-linecap="round" stroke-linejoin="round">
